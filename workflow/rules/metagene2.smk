...
rule compute_matrix:
    ...
    shell:
        ...
        awk -F'\t' -v OFS='\\t' '
          FNR==NR&&FNR>1{{
            for (i=(({params.before}/{params.bin_size})+4); i<=((({params.before}+{params.body_length})/{params.bin_size})+3) ; i++) {{
              sum[FNR]+=$i ;
            }} ; 
            size[FNR]=sum[FNR]*{params.bin_size}/{params.body_length};
            total+=(sum[FNR]/(FNR-1)) ;
            next
          }} 
          FNR<NR&&FNR==1 {{
          ...
          FNR<NR&&FNR>1 {{
            for (i=4;i<=NF;i++) {{
              $i=(size[FNR]>0?$i/size[FNR]*total:0) ;
            }} ;
            print
