rule differential_plots:
    input:
        ...
        sense_mx = lambda wildcards: expand("matrices/{sample.sample}/{sample.unit}/{{reference}}/{{prefix}}.{sample.stranded}/{{lineage}}_{{valid}}.plot-{plot_md5}.{{tag}}.{{feature}}.sense.sorted_matrix.gz",
            sample=results[results.experiment==wildcards.experiment].itertuples(),
            plot_md5=features.loc[wildcards.feature,"plot_md5"],
        ),
        antisense_mx=lambda wildcards: [] if features.loc[wildcards.feature,"antisense"] else expand("matrices/{sample.sample}/{sample.unit}/{{reference}}/{{prefix}}.{sample.stranded}/{{lineage}}_{{valid}}.plot-{plot_md5}.{{tag}}.{{feature}}.antisense.sorted_matrix.gz",
            sample=results[results.experiment==wildcards.experiment].itertuples(),
            plot_md5=features.loc[wildcards.feature,"plot_md5"],
        ),
