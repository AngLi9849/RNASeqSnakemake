rule differential_plots:
    input:
        ...
        sense_mx = lambda wildcards: list(dict.fromkeys(expand("matrices/{sample.sample_name}/{sample.unit_name}/{{reference}}/{{splice}}{{prefix}}.{sample.stranded}/{{lineage}}_{{valid}}.plot-{plot_md5}.{{tag}}.{{feature}}.sense.sorted_matrix.gz",
            sample=results[results.experiment==wildcards.experiment].itertuples(),
            plot_md5=features.loc[wildcards.feature,"plot_md5"],
        ))),
        antisense_mx=lambda wildcards: [] if not features.loc[wildcards.feature,"antisense"] else list(dict.fromkeys(expand("matrices/{sample.sample_name}/{sample.unit_name}/{{reference}}/{{splice}}{{prefix}}.{sample.stranded}/{{lineage}}_{{valid}}.plot-{plot_md5}.{{tag}}.{{feature}}.antisense.sorted_matrix.gz",
            sample=results[results.experiment==wildcards.experiment].itertuples(),
            plot_md5=features.loc[wildcards.feature,"plot_md5"],
        ))),
    ...
    params:
        ...
        samples= lambda wildcards: list(dict.fromkeys(expand("{sample.sample_name}_{sample.unit}",
            sample=results[results.experiment==wildcards.experiment].itertuples(),
        ))), 
    resources:
        mem="16G",
