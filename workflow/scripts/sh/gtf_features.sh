        awk -F'\\t' -v OFS='\\t' '
          $8=="exon" && match($0,/transcript_id "([^"]*)".*exon_number "([^"]*)".*transcript_support_level "([^"]).*".*/,a) {{ 
            if (a[3]==1) {{ 
              ...
              print $1, $2, $3, $4, $5, $6, "transcript", a[1], b[1], "exon",a[2] 
            }}
          }} 
          $8!="exon"&&$8!="CDS"&&match($0,/transcript_id "([^"]*)".*transcript_support_level "([^"]).*".*/,a) {{
            if (a[2]==1) {{
              if ($0~"transcript_name") {{
                match($0,/transcript_name "([^"])".*/,b)
              }}
              ...
              print $1, $2, $3, $4, $5, $6, "transcript", a[1], b[1], c, 1 }}}}
        ' {input.bed} |
        
        sort -k1,1 -k8,8 -k9,9 -k2,2n -k3,3n > {output.transcripts} && 
        ...
        awk -F'\\t' -v OFS='\\t' '
          FNR==NR{{name[$1]=$2; type[$1]=$3 ; num[$1]=$4>1?"Multiexonic":"Monoexonic"}} 
          FNR<NR{{printf "%s\\t%s\\t%s\\t%s\\t%s %s %s\\t%s\\t%s %s%s%s%s\\n", $0,num[$4],type[$4],name[$4],name[$4],"exon",$8,"variant",name[$4],"Ex",$8,"var",$10}}
        ' {input.gene_type} - > {output.exon} &&
