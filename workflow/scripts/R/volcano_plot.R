...
volcano_plot <- volcano +
  ...
  geom_text_repel(
    mapping=aes(label=ifelse((feature_name %in% goi), as.character(feature_name),ifelse((p_rank<=volc_n | lfc_rank <= volc_n) & padj < sig_p ,as.character(feature_name),NA))),
    size=ifelse(expr_i$feature_name %in% goi, 4.0, 3.0),
    color=ifelse(expr_i$feature_name %in% goi, "white", "grey20"),
    fontface=ifelse(expr_i$feature_name %in% goi, 2, 1),
    segment.color=ifelse(expr_i$feature_name %in% goi, "black", "grey20"),
    segment.alpha=ifelse(expr_i$feature_name %in% goi, 1, 0.5),
    bg.color=ifelse(expr_i$feature_name %in% goi, "black", NA),
    bg.r=ifelse(expr_i$feature_name %in% goi, 0.2, NA),
