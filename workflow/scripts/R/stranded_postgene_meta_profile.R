...
sense_matrices_read <- lapply(sense_matrices_list,function(x) {read.table(x,sep='\t',row.names=3,header=F,skip=1,check.names=FALSE)})
sense_matrices_df <- lapply(sense_matrices_read,function(x) {x <- x[3:ncol(x)]})
...
names(sense_matrices_df) <- sense_names
...
tts <- as.numeric(snakemake@params[["tts"]])
postgene <- as.numeric(snakemake@params[["postgene"]])
...
head(sense_trimmed_mean,10)

colnames(sense_trimmed_mean) <- sense_names
sense_trimmed_mean$Position <- c((1:nrow(sense_trimmed_mean)*bin_size-(tts+bin_size)))
meta_mean <- melt(sense_trimmed_mean,id.vars="Position")
meta_mean$Sample <- meta_mean$variable
...
xlim <- c((0-tts),(postgene))
xbrks <- c(0,if(tts>0)(signif(0-(tts*1.5),1)/2) else NULL,if(postgene>0)(signif(postgene*1.5,1)/2) else NULL, if(postgene>0)(signif(postgene*1.5,1)/4) else NULL)

names(xbrks) <- c("TTS",if(tts>0)(paste("-",signif(tts*1.5,1)/2,sep="")) else NULL,if(postgene>0)(paste("+",signif(postgene*1.5,1)/2,sep="")) else NULL,if(postgene>0)(paste("+",signif(postgene*1.5,1)/4,sep="")) else NULL)

gene_number <- paste("n=",format(nrow(sense_matrices_df[[1]]),big.mark=",",scientific=F),sep="")

title <- paste(prefix, " ", feature," TTS-Postgene",sep="")
