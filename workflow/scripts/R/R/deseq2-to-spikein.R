...
# Generate logical vector to identify spikein genes in count matrix
spikein_gene <- rownames(dds) %in% read.csv(file=snakemake@input[["spikein_bed"]],sep="\t")[[4]]
# Estimate size factors from spikein genes:
...
write.table(data.frame(sizeFactors(dds),1/sizeFactors(dds)),file=snakemake@output[[3]],sep='\t',row.names = TRUE, quote = FALSE, col.names = (c("sample_name\tspikein_size_factor","scale_factor")))
# Write dds as RDS
